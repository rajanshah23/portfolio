      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {certifications.map((cert, index) => (
            <div
              key={cert.id}
              className={`bg-gradient-to-br from-${cert.color}-50 to-${cert.color === 'blue' ? 'indigo' : cert.color === 'green' ? 'emerald' : 'pink'}-50 rounded-xl p-6 border border-${cert.color}-100 transform hover:-translate-y-2 transition-all duration-300 ${
                inView ? 'animate-fade-in' : 'opacity-0'
              }`}
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              <div className="flex items-start space-x-4">
                <div className={`flex-shrink-0 w-16 h-16 bg-${cert.color}-500 rounded-lg flex items-center justify-center`}>
                ...
                <div>
                  ...
                  <div className="flex items-center text-sm text-gray-500 mb-4">
                    ...
                    <span>{cert.expiryDate ? `Expires: ${cert.expiryDate}` : 'No Expiration'}</span>
                  </div>
                  {cert.link && (
                    <a
                      href={cert.link}
